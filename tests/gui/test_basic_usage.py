...
def click_through_mainwindow(qtbot: QtBot, win: SPCalMainWindow):
    ...
    for action in win.toolbar_view.view_actions.values():
        qtbot.mouseClick(win.toolbar_view.widgetForAction(action), QtCore.Qt.MouseButton.LeftButton)
...
def test_gui_no_data(qtbot: QtBot, test_locales):
    ...
    click_through_mainwindow(qtbot, win)
...
def test_gui_single_quad_data(qtbot: QtBot, test_locales, test_data_path):
    ...
    # npz = np.load(Path(__file__).parent.parent.joinpath("data/agilent_au_data.npz"))
    # data = np.array(npz["au50nm"], dtype=[("Au", float)])
    df = SPCalTextDataFile.load(test_data_path.joinpath("text/agilent_au50nm.csv"), skip_rows=4)
