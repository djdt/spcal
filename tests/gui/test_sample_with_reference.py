...
def test_sample_with_reference(qtbot: QtBot):
    ...
    assert window.sample.detections["Au"].size == 3184
    assert np.isclose(
    ...
    assert window.sample.io["Au"].count.value() == 3184
    assert window.sample.io["Au"].count.error() == np.round(np.sqrt(3184), 0)
    assert np.isclose(
        window.sample.io["Au"].background_count.value(), 0.2724, atol=1e-4
    )
    ...
    assert window.reference.detections["Au"].size == 9793
    assert np.isclose(
    ...
    assert np.isclose(window.reference.io["Au"].efficiency.value(), 0.05563, atol=1e-5)
    ...
    assert window.results.io["Au"].count.value() == 3184
    assert window.results.io["Au"].count.error() == np.round(np.sqrt(3184), 0)
    ...
    assert np.isclose(window.results.io["Au"].number.baseValue(), 8.033e8, rtol=1e-4)
